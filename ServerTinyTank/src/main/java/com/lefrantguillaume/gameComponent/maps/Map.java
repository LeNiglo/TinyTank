...
public class Map {
    ...
    private boolean detectCollision(List<Obstacle> world, List<Rectangle2D> obstacles, Rectangle2D object) {
        for (int i = 0; i < world.size(); ++i) {
            Rectangle2D current = new Rectangle2D.Double(world.get(i).getXByIndex(0), world.get(i).getYByIndex(0), world.get(i).getSizeXByIndex(0), world.get(i).getSizeYByIndex(0));
        ...
        for (int i = 0; i < respawnPoints.size(); ++i) {
            for (int i2 = 0; i2 < 10; ++i2) {
                ...
                for (int i3 = 0; i3 < collisionObject.size(); ++i3) {
                    Rectangle2D object = new Rectangle2D.Double(x - collisionObject.get(i).getShiftOrigin().getKey(), y - collisionObject.get(i).getShiftOrigin().getValue(),
